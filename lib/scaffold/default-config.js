...
/**
 * Will return the path and default rogercore-node configuration. It will search for the
 * configuration file in the "~/.theholyroger" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.theholyroger');
  var defaultConfigFile = path.resolve(defaultPath, './rogercore-node.json');
      ...
      servicesConfig: {
        bitcoind: {
          spawn: {
            ...
            exec: path.resolve(__dirname, '../../bin/theholyrogerd')
          }
        }
      }
    };
