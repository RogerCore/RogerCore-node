...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'rogercore-lib': '^' + bitcore.version,
    'rogercore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/rogercore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the rogercore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the rogercore-node directory and configuration
